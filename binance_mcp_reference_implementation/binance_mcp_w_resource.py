"""
binance_mcp_w_resource.py

This script is a variant of the reference Binance MCP server implementation with resource-based features.

Key features:
- Implements the MCP (Model Context Protocol) server for Binance price data, with additional resource management or data enrichment features.
...
Typical usage:
- Run as a standalone MCP server with resource/data enrichment capabilities.
...
def symbol_map() -> str:
    """
    Gets a symbol based on a friendly name

    Args:
        str: The friendly symbol of the crypto asset to get symbol name of
